...
public class homework3 {
	...
	public static void main(String[] args) {
		...
		System.out.println();
		String s = new String(); int year; int month=11;
		...
		for (int i = 0;i<=11;i++){
			...
			aa[year][i]=new int[31];
			case 3: case 5: case 8: case 10:
			aa[year][i]=new int[30];
			case 1:
				if (year % 4 == 0){
					if (year % 100 == 0){
						if (year % 400 == 0){
							aa[year][1]=new int[29];
							// 29
						}else{
							aa[year][1]=new int[28];
							// 28
						}
					}else{
						aa[year][1]=new int[29];
		...
		for(int y = 1752 ; y<aa.length;y++){
			for(int m = 0 ; m < aa[y].length;m++){
				for(int d = 0; d< aa[year][m].length;d++){
					aa[y][m][d]=d+1;
